...
class AppModel extends Model {
   ...
   function file_checktype($data){
	...
	function beforeSave(){
        ...
        $this->set("fecha_modificacion", $nowDate); 
       if ( !$this->exists() ) { 
                $this->set("fecha_creacion", $nowDate); 
        } 

		if(isset($this->data[$this->name]["apellido"])){
			$this->data[$this->name]["apellido"] = mb_strtoupper($this->data[$this->name]["apellido"], 'UTF-8');
		}
...
/*
		if(isset($this->data["Registro"]) && isset($this->data["Registro"]["nombres"])){
			$this->data["Registro"]["nombres"] = mb_strtoupper($this->data["Registro"]["nombres"], 'UTF-8');
		}
*/
		return true;
	}
