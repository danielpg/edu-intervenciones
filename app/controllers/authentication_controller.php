	...
	function beforeFilter(){
		...
		$this->set("title_for_layout","Iniciar Sesion");	
 		...
 		if($this->action!='logout' && Credentials::hasCredentials()==true){
 			$this->redirect(array('controller'=>'valores','action'=>'index'));
 			exit;
 		}
		...
		if (!empty($this->data)) {
			...
			if ($login->authenticate($this->data)) {
				if ($this->params["isAjax"]) {
					...
					echo json_encode(array("error"=>0,"url_redirect"=>Router::url(array('controller'=>'valores','action'=>'index'))));exit;
				} else {
				...
				if ($this->RequestHandler->isAjax()) {
					...
					$this->_flash(__('Usuario y/o contrase&ntilde;a invalidos.', true),'default',array('class'=>'box_notice'));
