...
<div  style="text-align:center">
	<div align="center">
		...
		<div style="font-size:18px;font-family:'Arial Black';text-align:center;margin-bottom:10px">INTERVENCIONES</div>
		...
		<td xalign="center">
				...
				<form  action="<?php echo $html->url(array('controller'=>'authentication','action'=>'login')) ?>" id="ajax_login" method="post" onsubmit="javascript:submitForm('#ajax_login');return false;">
					...
					<div id="form_errors" style=" color:red; font-weight:bold">
								<?php 
						
								 if($messages = $session->read('Message.multiFlash')) {
										//var_dump($messages);
                						foreach($messages as $k=>$v){
											echo "<div style='color:red' class='box_".$v["class"]."'>".$v["message"]."</div>";
